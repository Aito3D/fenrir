"""Tests for daemon.main._deploy_ssh_key — Fenrir key sync.

Background: Fenrir generates an ed25519 keypair under its data dir and ships
the public half to the SpoolBuddy daemon over the registration/heartbeat
response. The daemon writes that key into ~/.ssh/authorized_keys so Fenrir
can SSH in to drive remote updates. Whenever Fenrir's keypair rotates (data
...
must drop the old entries and pick up the new one — otherwise:
  ...
  2. Stale Fenrir-tagged keys pile up over time, eroding the security
     boundary (any prior keypair Fenrir held is permanently authorized).
...
class TestDeploySshKey:
    ...
    def test_replaces_all_prior_fenrir_tagged_keys(self, tmp_path):
    ...
    def test_preserves_unrelated_user_keys(self, tmp_path):
        """Only Fenrir-tagged keys get replaced — user's own keys stay."""
        ssh_dir = tmp_path / ".ssh"
        ssh_dir.mkdir()
        auth_keys = ssh_dir / "authorized_keys"

...
- **Toolbar in the archives** (Done, Trash): only the active toggle (the way back) and the search box, widened to `lg:w-96`; the other archive toggle, Statistics, Import and New project are hidden. The count and print-backlog badges stay, since the archives still describe the live shop.

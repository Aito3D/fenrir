...
**Build under test:** either branch `feature/spoolman-inventory-ui` OR docker image `fenrir:spoolman-test_20260505` (both contain the merge with dev as of 2026-05-05)
**Issued:** 2026-05-05
...
5. If ANY of (a)/(b)/(c) is wrong, the row is a fail — even if Fenrir's own UI looks correct.
...
- [ ] Fenrir running, either built from branch `feature/spoolman-inventory-ui` **or** pulled from docker image `fenrir:spoolman-test_20260505`
...
```bash
# from your Fenrir admin shell, or via the API:
curl -s ${FENRIR_URL}/api/v1/inventory/spools | jq 'length'
curl -s ${FENRIR_URL}/api/v1/inventory/assignments | jq 'length'
curl -s ${FENRIR_URL}/api/v1/spoolman/inventory/spools | jq 'length'   # spoolman mode
curl -s ${FENRIR_URL}/api/v1/spoolman/inventory/slot-assignments | jq 'length'
```
...
> Empty slot = printer reports `tray_type=""`. Bambu firmware silently drops MQTT for these, so Fenrir persists the assignment and replays MQTT when the spool is physically inserted.
...
| D12 | API key can no longer read settings *(skip if you don't use API keys)* | Settings → API Keys → create a key. From a terminal: `curl -H "Authorization: Bearer <key>" <fenrir-url>/api/v1/settings` — must return the settings JSON, not "403 Forbidden". | _ | _ |
...
- Fenrir version (`/api/v1/version`)
- Printer model + firmware
- Slicer + version
- A screenshot of the slicer's slot detail modal (for any B/C section failure)
- Fenrir logs from the relevant 60s window (`docker logs fenrir --since 1m`)

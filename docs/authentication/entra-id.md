...
This guide shows how to configure Fenrir's OIDC integration with **Microsoft Azure Entra ID** (formerly Azure Active Directory).
...
- Fenrir ≥ 1.0 with OIDC enabled (Settings → Authentication → OIDC Providers).
...
- **`preferred_username`** (recommended) — Entra ID always populates this with the UPN (e.g. `user@contoso.com`). Fenrir treats it as an email-shaped identifier and skips the `email_verified` check entirely.
...
Azure Entra ID issues access tokens that expire after 1 hour by default. Fenrir exchanges the OIDC code for its own JWT at callback time, so the Entra token lifetime does not affect the Fenrir session length. You can adjust Fenrir's `JWT_ACCESS_TOKEN_EXPIRE_MINUTES` env var independently.
...
|---|---|
| Login redirects to Entra but returns "OIDC login failed" | Redirect URI mismatch — check the URI registered in Azure exactly matches Fenrir's callback URL, including scheme and trailing path. |
...
| "Invalid client" error from Azure | Client secret has expired or was copied incorrectly. Rotate the secret in Azure and update the provider in Fenrir. |

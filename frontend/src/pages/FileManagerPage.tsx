...
import { UploadFilesModal as UploadFilesModalShared } from '../components/UploadFilesModal';
...
// Upload Modal — uses shared UploadFilesModal component
function UploadModal({ folderId, onClose, onUploadComplete, t }: {
  folderId: number | null;
  onClose: () => void;
  onUploadComplete: () => void;
  t: TFunction;
}) {
  const handler: import('../components/UploadFilesModal').UploadHandler = {
    upload: async (file, options) => {
      if (file.name.toLowerCase().endsWith('.zip')) {
        const result = await api.extractZipFile(file, folderId, options.preserveZipStructure, options.createFolderFromZip, options.generateStlThumbnails);
        if (result.errors.length > 0 && result.extracted === 0) {
          throw new Error(`${result.errors.length} files failed`);
        }
        return { extractedCount: result.extracted };
      }
      await api.uploadLibraryFile(file, folderId, options.generateStlThumbnails);
    },
  };

  return (
    <UploadFilesModalShared
      handler={handler}
      onClose={onClose}

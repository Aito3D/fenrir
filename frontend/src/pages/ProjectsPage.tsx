...
export function ProjectsPage() {
  ...
  const renderProjectTree = (project: ProjectListItem, depth: number, seen: Set<number>) => {
    ...
    // `animate-rise-lg` on both return paths, not just the leaf: the fork's
    // stagger applies to every card the grid lays out, and a parent group
    // that skipped it would be the one tile that snaps in while its
    // neighbours rise.
    if (children.length === 0) return <div key={project.id} className="animate-rise-lg">{card}</div>;
    ...
    return (
      ...
      <div key={project.id} className={expanded ? 'col-span-full space-y-4 animate-rise-lg' : 'space-y-4 animate-rise-lg'}>
        {card}
        <div
  ...
  const handleExportAll = async () => {
    try {
      ...
      a.href = url;
      a.download = `fenrir_projects_${new Date().toISOString().split('T')[0]}.json`;
      a.click();
  ...
  return (
    <div className="p-4 md:p-8 space-y-8">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        {childrenByParent.size > 0 && (
          <button
            ...
            className={`btn-press flex items-center gap-2 px-3 py-1.5 text-sm rounded-lg transition-colors ${
              collapseSubProjects
                ? 'bg-bambu-green/20 text-bambu-green'
      ...
      {isLoading ? (
        ...
        /* Upstream's project tree (nested sub-projects) keeps the fork's
           staggered entrance: `stagger-parents` here, `animate-rise-lg` on
           each node inside renderProjectTree. The two are independent — the
           tree decides WHAT renders, the stagger only how it arrives.
           A plain block comment rather than a JSX one: this sits in a ternary
           branch, which takes a single expression, and a JSX comment would
           make it two children. */

...
export function CameraPage() {
  ...
  // Revoke stale snapshot blob URL on unmount
  const snapshotBlobUrlRef = useRef<string | null>(null);
  useEffect(() => {
    return () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (snapshotBlobUrlRef.current) URL.revokeObjectURL(snapshotBlobUrlRef.current);

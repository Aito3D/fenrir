 ...
 *  A suppressed entry (avg_watts null — T-046 cross-window guard)
 *  contributes nothing: it's excluded here exactly like a below-MIN_SAMPLE
 *  entry, from both the aggregation numerator and the sample denominator. */
function pickPowerDraw(insights: CalculatorInsights, printer: CalculatorPrinter | undefined) {
  const matches = matchPrinters(insights.power_by_printer, printer).filter((m) => m.avg_watts !== null);
 ...
 *  A suppressed entry (hours_per_day null — T-044 cross-window guard)
 *  contributes nothing: it's excluded here exactly like a below-MIN_SAMPLE
 *  entry, from both the aggregation numerator and the day-weighted
 *  denominator. */
function pickDailyUsage(insights: CalculatorInsights, printer: CalculatorPrinter | undefined) {
  const matches = matchPrinters(insights.usage_by_printer, printer).filter((m) => m.hours_per_day !== null);
  if (!printer || matches.length === 0) return null;

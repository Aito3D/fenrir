  ...
  return (
    <section data-testid="aito-stats-brief" className="space-y-4">
      <div className="space-y-1.5">
        <p className="text-[11px] uppercase tracking-[0.06em] text-bambu-gray">
...
function List({ kind, heading, rows, onOpen }: { kind: ListKind; heading: string; rows: Row[]; onOpen?: (id: number) => void }) {
  ...
  return (
    <div data-testid={`aito-brief-${kind}`} className="space-y-2">
      <h3 className="flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.06em] text-bambu-gray-light">
        <span aria-hidden="true" className={`inline-block h-2 w-2 rounded-full ${TONE[kind].dot}`} />
        {heading}
        <span className="ml-auto text-white tabular-nums">{rows.length}</span>
      </h3>
      {rows.length === 0 ? (
        <p className="rounded-lg border border-dashed border-bambu-dark-tertiary px-3 py-2.5 text-xs text-bambu-gray">{t(`aito.stats.brief.${kind}Empty`)}</p>
      ) : (
          ...
          {shown.map((row) => (
            <li key={row.id}>
              <button
                ...
                className="grid w-full grid-cols-[minmax(0,1fr)_auto] items-center gap-x-3 gap-y-0.5 rounded-lg bg-bambu-dark-secondary px-3 py-2 text-left transition-colors hover:bg-bambu-dark-tertiary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-bambu-green/40"
              >
                <span className="truncate text-[13px] font-semibold text-white">{row.who}</span>
                <span className={`text-[13px] font-semibold tabular-nums ${TONE[kind].wait}`}>{row.wait}</span>
                <span className="truncate text-xs text-bambu-gray-light">{row.what}</span>
                <span className="text-right text-xs text-bambu-gray tabular-nums">{row.amount ?? ''}</span>
              </button>
            </li>
          ))}
          {rows.length > ROW_CAP && (
            <li className="px-3 text-xs text-bambu-gray">{t('aito.stats.brief.andMore', { count: rows.length - ROW_CAP })}</li>
          )}
        </ul>
      )}
    </div>

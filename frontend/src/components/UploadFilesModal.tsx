...
export interface UploadHandler {
  /**
   * Called for each file. Return { extractedCount } for ZIP files.
   * Throw to mark as error.
   */
  upload: (
    file: File,
    options: { preserveZipStructure: boolean; createFolderFromZip: boolean; generateStlThumbnails: boolean },
  ) => Promise<{ extractedCount?: number } | void>;
}
...
export interface UploadFilesModalProps {
  ...
  /** When true, only allow a single file at a time */
  singleFile?: boolean;
...
export function UploadFilesModal({
  ...
  const addFiles = (newFiles: File[]) => {
    const uploadFiles: UploadFileEntry[] = newFiles.map((file) => ({
      file,
      status: 'pending',
    ...
    if (singleFile) {
      setFiles(uploadFiles.slice(0, 1));
    } else {
  ...
  const handleUpload = async () => {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      try {
        const result = await handler.upload(files[i].file, {
          preserveZipStructure,
          createFolderFromZip,
          generateStlThumbnails,
        });
        setFiles((prev) =>
          prev.map((f, idx) =>
            idx === i
              ? { ...f, status: 'success', extractedCount: result?.extractedCount }
              : f
          )
        );
      } catch (err) {
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-bambu-dark-secondary rounded-lg w-full max-w-lg border border-bambu-dark-tertiary">
        <div className="p-4 border-b border-bambu-dark-tertiary flex items-center justify-between">
          <h2 className="text-lg font-semibold text-white">{title ?? t('fileManager.uploadFiles')}</h2>

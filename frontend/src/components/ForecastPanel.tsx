...
import type { InventorySpool, ShoppingListItem } from '../api/client';
...
export function ForecastPanel({ spools }: { spools: InventorySpool[] }) {
  ...
  const groups = useMemo((): SkuGroup[] => groupSpoolsBySku(spools), [spools]);

  const forecasts = useMemo(
    (): SkuForecast[] => computeSkuForecasts(groups, skuSettingsList, usageHistory, globalLeadTime),
    [groups, skuSettingsList, usageHistory, globalLeadTime],
  );
...
function AddToCartModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-overlay-in">
      <div className="bg-bambu-dark-secondary rounded-2xl border border-bambu-dark-tertiary w-full max-w-sm shadow-2xl animate-modal-in">

...
import { Printer, Archive, ListOrdered, BarChart3, Calculator, Cloud, Settings, Sun, Moon, Monitor, ChevronLeft, ChevronRight, Keyboard, Github, ArrowUpCircle, Wrench, FolderKanban, FolderOpen, Kanban, X, Menu, Info, Plug, Bug, LogOut, Key, Loader2, Disc3, ShieldAlert, Globe, Bell, type LucideIcon } from 'lucide-react';
...
export const defaultNavItems: NavItem[] = [
  ...
  { id: 'calculator', to: '/calculator', icon: Calculator, labelKey: 'nav.calculator' },
      ...
      {isSidebarCompact && !fullscreen && (
        <header className={`${playIntro ? 'animate-topbar-in' : ''} fixed top-0 left-0 right-0 z-40 h-14 bg-bambu-dark-secondary border-b border-bambu-dark-tertiary flex items-center px-4`}>
          ...
          {/* Bug report — the compact-layout home of the floating bubble.
              Upstream's, and required: BugReportBubble now hides its own
              floating disc below this breakpoint (showTrigger), so without
              this button the bug report is unreachable on a compact layout. */}
          <button

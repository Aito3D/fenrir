...
// flow). Split out of the former CalculatorSettingsPanels.tsx (T-078). The
// Zoho sync walk itself (chunking/timeout/progress persistence) lives in
// useZohoFilamentSync (T-002) so this file owns only listing/form/dialog
// wiring.
...
import { useZohoFilamentSync } from './useZohoFilamentSync';
...
export function CalculatorFilamentsPanel({
  ...
  const { syncProgress, syncSummary, syncError, syncTimedOut, runSync } = useZohoFilamentSync(filaments);

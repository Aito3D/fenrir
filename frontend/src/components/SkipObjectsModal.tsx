...
export function SkipObjectsModal({ printerId, isOpen, onClose }: SkipObjectsModalProps) {
  ...
  return (
    <>
      ...
      >
        <div className="absolute inset-0 bg-black/55 animate-overlay-in" />
        <section
          className="relative z-10 flex max-h-[88vh] w-full max-w-[980px] flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-2xl dark:border-bambu-dark-tertiary dark:bg-bambu-dark animate-modal-in"
      ...
      {enlarged && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/90 p-8 animate-overlay-in" onClick={() => setEnlarged(false)}>
          <button type="button" onClick={() => setEnlarged(false)} className="absolute right-4 top-4 rounded p-2 text-white/70 hover:text-white">
            <X className="h-6 w-6" />
          </button>
          <div className="aspect-square max-h-[86vh] w-full max-w-[86vh] animate-modal-in" onClick={(event) => event.stopPropagation()}>
            {renderPlate(true)}
          </div>
        </div>
      )}

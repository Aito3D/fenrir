...
describe('ToastContext auto-dismiss timing by type', () => {
  ...
  function showAndAdvance(
    ...
    // FORK DIVERGENCE: upstream's toast is removed from the DOM the instant
    // its timer fires, so it asserted on the node being absent. This fork
    // slides the toast out over ~150ms first (ToastShell / beginLeave), so at
    // exactly the dismiss boundary the node is still mounted — carrying
    // data-state="leaving". "Dismissed" here therefore means the dismissal has
    // STARTED, which is the behaviour upstream's timings actually pin. Testing
    // for absence instead would only be asserting the animation duration.
    const node = queryByText(`a ${type} message`);
    const stillThere = node !== null && node.closest('[data-state="leaving"]') === null;
    unmount();
    return stillThere;
  }

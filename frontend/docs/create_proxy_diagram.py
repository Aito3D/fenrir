#!/usr/bin/env python3
"""
Create a professional network architecture diagram for Fenrir Virtual Printer Proxy Mode.
...
def create_diagram():
    ...
    # Subtitle
    subtitle = "Secure remote printing through Fenrir"
    ...
    # Fenrir section (center-right)
    fenrir_x = 700
    fenrir_box = [560, 140, 840, 500]
    ...
    draw.text((remote_x, section_y + 100), "Connects to Fenrir", font=fonts['small'], fill=TEXT_LABEL, anchor="mm")
    ...
    # === FENRIR SERVER ===
    draw_rounded_rect(draw, fenrir_box, 12, fill=CONTAINER_BG, outline=BAMBU_GREEN_DIM, width=2)
    draw.text((fenrir_x, 165), "FENRIR SERVER", font=fonts['label'], fill=BAMBU_GREEN, anchor="mm")

    # Server icon
    draw_server_icon(draw, fenrir_x, section_y - 50, 70, BAMBU_GREEN)
    draw.text((fenrir_x, section_y + 20), "TLS Proxy", font=fonts['heading'], fill=TEXT_PRIMARY, anchor="mm")

    # Incoming ports (left side of Fenrir)
    draw.text((fenrir_x, section_y + 70), "LISTEN PORTS", font=fonts['small'], fill=TEXT_LABEL, anchor="mm")
    draw_rounded_rect(draw, [fenrir_x - 55, section_y + 85, fenrir_x + 55, section_y + 130],
                      6, fill=(35, 35, 45), outline=CONTAINER_BORDER, width=1)
    draw.text((fenrir_x, section_y + 98), "FTP", font=fonts['small'], fill=TEXT_SECONDARY, anchor="mm")
    draw.text((fenrir_x, section_y + 115), "990", font=fonts['port'], fill=BAMBU_GREEN, anchor="mm")

    draw_rounded_rect(draw, [fenrir_x - 55, section_y + 140, fenrir_x + 55, section_y + 185],
                      6, fill=(35, 35, 45), outline=CONTAINER_BORDER, width=1)
    draw.text((fenrir_x, section_y + 153), "MQTT", font=fonts['small'], fill=TEXT_SECONDARY, anchor="mm")
    draw.text((fenrir_x, section_y + 170), "8883", font=fonts['port'], fill=BAMBU_GREEN, anchor="mm")
    ...
    # TLS badge between Fenrir and printer
    ...
    # Flow description
    draw.text((WIDTH // 2, info_y), "← Slicer traffic encrypted and relayed through Fenrir to your printer →",
...
def main():
    ...
    output_path = Path("/opt/claude/projects/fenrir/docs/images/proxy-mode-diagram.png")
    ...
    frontend_path = Path("/opt/claude/projects/fenrir/frontend/docs/proxy-mode-diagram.png")

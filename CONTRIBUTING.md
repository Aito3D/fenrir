...
Thank you for your interest in contributing to Fenrir! This document provides guidelines and instructions for contributing.
...
- **[fenrir-wiki](https://github.com/maziggy/bambuddy-wiki)** — end-user guide (installation, configuration, feature walkthroughs, reference)
- **[fenrir-website](https://github.com/maziggy/bambuddy-website)** — marketing site (updated only when the change affects public claims or feature lists)
...
1. Open your code PR here in `fenrir`
2. Open companion PR(s) in `fenrir-wiki` and/or `fenrir-website`
...
- **Wiki** (`fenrir-wiki`) — `pip install -r requirements.txt && mkdocs serve` — live-reload on `http://localhost:8000`
- **Website** (`fenrir-website`) — static HTML/CSS, open the changed file directly or serve with `python -m http.server`
...
Fenrir has an optional authentication system. When auth is enabled, API endpoints are protected by granular permissions.
...
Thank you for contributing to Fenrir!

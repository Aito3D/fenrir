"""Build script for the Fenrir Windows installer.
...
runner) — it pip-installs Fenrir's deps against the embedded Python it
...
def stage_embedded_python() -> Path:
    ...
    # package. Fenrir's requirements.txt hits this with pyftpdlib 2.2.0
...
def install_requirements(python_dir: Path) -> None:
    """Install Fenrir's requirements.txt into the embedded Python."""
...
def stage_backend(frontend_dist: Path) -> None:
    ...
    The runtime layout under STAGING/app/ mirrors a Fenrir checkout:
...
def _read_app_version() -> str:
    ...
    source used by every other Fenrir surface — FastAPI OpenAPI title,
...
def _resolve_installer_version() -> str:
    ...
    ``fenrir-0.2.5b1-daily.20260610-windows-x64-setup.exe``, not
    ``fenrir-v0.2.5b1-...``.
    """
...
def main() -> int:
    ...
    if sys.platform != "win32" and not args.allow_non_windows:
        ...
        log("pip-installs Fenrir's requirements.txt against it — both")

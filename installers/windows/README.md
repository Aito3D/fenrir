# Fenrir Windows Installer

Builds a self-contained Windows installer (`.exe`) for Fenrir: embedded
...
- **Install target:** `C:\Program Files\Fenrir\`
- **Data target:** `C:\ProgramData\Fenrir\data\` (preserved on uninstall by default)
- **Logs target:** `C:\ProgramData\Fenrir\logs\`
...
launcher in v1 — browser-as-UI matches every other Fenrir platform.
...
- **Spoolman:** explicitly NOT bundled in v1. Users who want Spoolman
  install it separately. Fenrir internal-inventory mode is the default
  on Windows.

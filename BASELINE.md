...
- GATE AMENDMENT (iter-8, disclosed): the gate's test glob missed `src/__tests__/components/TaskEditor.test.tsx`, leaving ALL of TaskEditor.tsx's interactive code outside the measured set (a pre-existing blind spot). The gate run now ALSO includes that test path. Re-baselined with the extended gate at loop-7 vs HEAD(T-024): 306 missed at both — no coverage was lost; the old-gate 349→356 bump was purely the blind spot. Extended-gate ratchet from here: ≤306 missed. Append `'src/__tests__/components/TaskEditor.test.tsx'` to the gate command below.

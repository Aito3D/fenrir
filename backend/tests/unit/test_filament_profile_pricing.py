...
def test_writes_the_array_of_one_string_form():
    ...
    updated, changed = apply_filament_cost(content, 19.9)
...
def test_tolerates_a_bare_scalar_cost():
    ...
    content = json.dumps({"filament_cost": "19.90"}, indent=4)
    _, changed = apply_filament_cost(content, 19.9)
    assert changed is False
...
def test_malformed_json_is_left_alone():
    updated, changed = apply_filament_cost("{not json", 19.9)
    assert changed is False
    assert updated == "{not json"


def test_non_object_json_is_left_alone():
    updated, changed = apply_filament_cost("[1, 2]", 19.9)
    assert changed is False
    assert updated == "[1, 2]"


def test_empty_content_is_left_alone():
    updated, changed = apply_filament_cost("", 19.9)
    assert changed is False
    assert updated == ""

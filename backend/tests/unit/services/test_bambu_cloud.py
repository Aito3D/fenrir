...
class TestBambuCloudTOTPVerification:
    ...
    @pytest.mark.asyncio
    async def test_verify_totp_uses_honest_fenrir_user_agent(self, cloud_service):
        """TOTP verification identifies as Fenrir, not as a browser.
        ...
        accepts ``Fenrir/X.Y.Z`` cleanly — the expected application-level
        ...
        with patch.object(cloud_service._client, "post", new_callable=AsyncMock) as mock_post:
            ...
            assert headers["User-Agent"].startswith("Fenrir/")
...
class TestCloudflareChallengeDetection:
    ...
    def test_message_mentions_bambu_lab_and_cloudflare(self):
        ...
        Cloudflare protection — not to Fenrir — so users know what to do."""
        from backend.app.services.bambu_cloud import _detect_cloudflare_challenge

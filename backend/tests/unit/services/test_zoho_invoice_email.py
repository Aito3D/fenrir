"""Books' invoice-email endpoints: prefill mapping and the send body."""
...
EMAIL_PAYLOAD = {
    ...
    "message": "success",
    "data": {
        "subject": "Facture INV-00087 de AITO 3D",
        "body": "<p>Bonjour,</p>",
        "to_contacts": [
            {
...
async def test_email_content_maps_subject_body_and_recipients(async_client, db_session):
    ...
    content = await zoho_service.get_invoice_email_content(db_session, "INV-7")

    assert content["subject"] == "Facture INV-00087 de AITO 3D"
...
@pytest.mark.asyncio
async def test_email_content_reads_the_invoice_endpoint_not_the_estimate_one(async_client, db_session):
    await _configure(async_client)
    seen: list[httpx.Request] = []
    zoho_service.transport = _transport(httpx.Response(200, json=EMAIL_PAYLOAD), seen)

    await zoho_service.get_invoice_email_content(db_session, "INV-7")

    assert "/invoices/INV-7/email" in str(seen[-1].url)


@pytest.mark.asyncio
async def test_email_invoice_posts_only_the_recipients(async_client, db_session):
    await _configure(async_client)
    seen: list[httpx.Request] = []
    zoho_service.transport = _transport(httpx.Response(200, json={"code": 0, "message": "sent"}), seen)

    await zoho_service.email_invoice(db_session, "INV-7", to_mail_ids=["jean-pierre@example.pf"])

    request = seen[-1]
    assert request.method == "POST"
    assert "/invoices/INV-7/email" in str(request.url)
    # Subject and body are absent on purpose: Books renders its own default
    # invoice template only when they are omitted.
...
async def test_send_rejection_maps_to_request_rejected(async_client, db_session):
    ...
    with pytest.raises(ZohoRequestRejected, match="No email address"):
        await zoho_service.email_invoice(db_session, "INV-7", to_mail_ids=["nobody@example.pf"])

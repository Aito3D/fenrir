...
class TestGetDesign:
    ...
    @pytest.mark.asyncio
    async def test_sends_honest_fenrir_user_agent(self, service):
        """The client identifies honestly as Fenrir, not as Firefox.

        Earlier iterations of this code stripped ``x-bbl-*`` Bambu-app
        identification headers but kept a Firefox User-Agent. Verified
        2026-05-12 that MakerWorld treats ``Fenrir/X.Y.Z`` identically to

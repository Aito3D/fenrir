...
class TestSanitizeCameraUrlValid:
    ...
    def test_hostname_allowed(self):
        ...
        # Mock DNS resolution since hostnames may not resolve in test environments
        fake_result = [(2, 1, 6, "", ("192.168.1.50", 0))]
        with patch("socket.getaddrinfo", return_value=fake_result):
            result = _sanitize_camera_url("http://my-camera.local/stream")
        assert result is not None
        # URL should use the resolved IP instead of hostname (DNS rebinding protection)

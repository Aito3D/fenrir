...
/etc/fenrir/local.toml that the appliance setup wizard writes.
...
def test_comment_only_file_returns_empty(tmp_path: Path):
    path = tmp_path / "local.toml"
    path.write_text("# Written by fenrir-wizard during firstboot.\n")
    assert read_local_toml(path) == {}
...
def test_full_config_parses(tmp_path: Path):
    path = tmp_path / "local.toml"
    path.write_text(
        "# Written by fenrir-wizard during firstboot.\n"

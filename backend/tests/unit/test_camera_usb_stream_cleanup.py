...
async def test_stop_endpoint_terminates_registered_external_process(monkeypatch):
    ...
    # This fork routes shared producers through SharedStreamHub rather than
    # upstream's get_subscriber_count/shutdown_broadcaster pair. An external USB
    # stream is never hub-owned, so say so — otherwise the endpoint skips it as
    # "someone else's process to manage".
    monkeypatch.setattr(camera._hub, "is_active", lambda _printer_id: False)
    ...
    camera._spawned_ffmpeg_pids[proc.pid] = time.monotonic()
...
async def test_cleanup_janitor_reaps_stale_external_usb_stream(monkeypatch):
    ...
    sid = "7-ext-deadbeef"
    # This fork tracks frame times per PRINTER (parsed off the stream_id prefix)
    # on the monotonic clock, not per stream on the wall clock.
    now = time.monotonic()
    camera._active_streams[sid] = proc
    camera._spawned_ffmpeg_pids[proc.pid] = now - 120  # spawned long ago
    camera._state.last_frame_times[7] = now - 60  # stale: no frames >30s
    ...
    try:
        ...
        camera._active_streams.pop(sid, None)
        camera._spawned_ffmpeg_pids.pop(proc.pid, None)
        camera._state.last_frame_times.pop(7, None)
        camera._disconnect_events.pop(sid, None)
...
def test_scan_matches_v4l2_ffmpeg(monkeypatch):
    ...
    # The scanner short-circuits off Linux (no /proc), so say we're on it —
    # the suite also runs on macOS dev machines.

"""S6: warn on unknown MFA_*/FENRIR_* env vars so typos like
...
@pytest.mark.unit
def test_unknown_fenrir_env_var_logs_info(monkeypatch, caplog):
    """An unrecognised FENRIR_* env var must also be logged."""
    monkeypatch.setenv("FENRIR_NEW_FEATURE", "v1")
    ...
    assert any("FENRIR_NEW_FEATURE" in rec.message for rec in caplog.records)

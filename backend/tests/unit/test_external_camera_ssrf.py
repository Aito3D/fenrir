...
class TestTheCamerasWeAllow:
    ...
    # Upstream does NOT resolve hostnames in the guard, and pinned that as
    # deliberate in a test named `test_a_hostname_is_not_resolved` plus two
    # hostname rows in the parametrize above. This fork does resolve, and
    # substitutes the resolved address into the URL it returns.
    #
    # The fork's behaviour is kept because it closes the gap between "we
    # checked the name" and "ffmpeg opened the name" — a rebinding answer
    # cannot land in between when the address is already pinned. The trade,
    # accepted knowingly, is that a camera named in DNS that cannot be
    # resolved is refused rather than attempted.
    #
    # These three tests replace upstream's. If a future upstream merge
    # reintroduces the originals, this divergence is being reverted — decide
    # that deliberately rather than letting a test conflict decide it.

    def test_a_named_camera_survives_and_is_pinned_to_its_address(self):
        with patch(
            "backend.app.services.external_camera.socket.getaddrinfo",
            return_value=[(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("192.168.1.50", 554))],
        ):
            assert _sanitize_camera_url("rtsp://cam.lan/live", RTSP_SCHEMES) == "rtsp://192.168.1.50/live"

    def test_a_name_that_resolves_to_loopback_is_refused(self):
        """The whole point of resolving: localtest.me is a public name whose
        A record is 127.0.0.1. Upstream lets it through and relies on nothing
        downstream catching it."""
        with patch(
            "backend.app.services.external_camera.socket.getaddrinfo",
            return_value=[(socket.AF_INET, socket.SOCK_STREAM, 6, "", ("127.0.0.1", 554))],
        ):
            assert _sanitize_camera_url("rtsp://localtest.me/live", RTSP_SCHEMES) is None

    def test_a_name_that_cannot_be_resolved_is_refused(self):
        with patch(
            "backend.app.services.external_camera.socket.getaddrinfo",
            side_effect=socket.gaierror("name or service not known"),
        ):
            assert _sanitize_camera_url("rtsp://nope.invalid/live", RTSP_SCHEMES) is None
...
class TestWhatTheGuardMustNotDestroy:
    ...
    def test_http_cameras_do_not_keep_their_basic_auth(self):
        """FORK DIVERGENCE. Upstream's version of this test is named
        `test_http_cameras_keep_their_basic_auth_too` and asserts the
        opposite — that userinfo survives on HTTP too.

        This fork strips it: credentials on a plain-HTTP URL travel in
        cleartext and get echoed into logs and downstream stream URLs, and
        the fork does not propagate them. RTSP/RTSPS keep theirs (see
        test_credentials_survive above), which is where cameras overwhelmingly
        carry a login anyway.

        The cost is real and accepted: an authenticated MJPEG camera
        configured with credentials in its URL will 401. If that turns up as
        a bug report, this is the line that decided it.
        """
        url = "http://admin:hunter2@192.168.1.50/stream.mjpg"
        assert _sanitize_camera_url(url, HTTP_SCHEMES) == "http://192.168.1.50/stream.mjpg"

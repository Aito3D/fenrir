"""The two contact-person columns on aito_projects (company cards only).
...
PERSON_COLUMNS = ("client_contact_person_id", "client_contact_name")

EXPECTED_TYPES = {
    "client_contact_person_id": "VARCHAR(50)",
    "client_contact_name": "VARCHAR(200)",
}


def test_model_declares_both_person_columns():
...
async def test_columns_default_to_null(db_session):
    project = AitoProject(description="no contact person", board_column="devis", position=0)
...
async def test_columns_round_trip(db_session):
    project = AitoProject(
        description="company card with a chosen contact person",
        board_column="devis",
        position=0,
        client_contact_person_id="cp-42",
        client_contact_name="Vaekehu VARNEY",
    )
    db_session.add(project)
    await db_session.commit()
    stored = (await db_session.execute(select(AitoProject).where(AitoProject.id == project.id))).scalar_one()
    assert stored.client_contact_person_id == "cp-42"
    assert stored.client_contact_name == "Vaekehu VARNEY"

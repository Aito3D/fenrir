...
def _enable_external_roots(monkeypatch, tmp_path):
    ...
    ``FENRIR_EXTERNAL_ROOTS`` env var (empty by default → feature
    ...
    """
    monkeypatch.setenv("FENRIR_EXTERNAL_ROOTS", str(tmp_path.parent))
...
class TestExternalFolderCreation:
    ...
    async def test_create_external_folder_nonexistent_path(self, async_client: AsyncClient, db_session, tmp_path):
        ...
        check, so the test path must be inside ``FENRIR_EXTERNAL_ROOTS``
    ...
    async def test_create_external_folder_outside_allowlist_blocked(self, async_client: AsyncClient, db_session):
        """Paths outside ``FENRIR_EXTERNAL_ROOTS`` are rejected (GHSA-r2qv I1).
...
class TestCrossBoundaryMove:
    ...
    in Fenrir's UI under the external folder but was never written to
    the NAS — so the SMB mount and Fenrir disagreed about what was
    actually there.
    """

...
async def device_heartbeat(
    ...
    # Fenrir's keypair rotates (data dir wiped, container recreated, etc.) —
...
async def nfc_tag_scanned(
    ...
    Routes the lookup to the inventory backend Fenrir is configured for:
    ...
    follow the inventory mode they selected in Fenrir settings.
    """
...
async def update_spool_weight(
    ...
    Routes the update to whichever inventory backend Fenrir is configured
...
async def check_daemon_update(
    ...
    """Check if the SpoolBuddy daemon needs updating to match the Fenrir backend version."""
...
async def trigger_daemon_update(
    ...
    Fenrir SSHes into the device, pulls the matching branch, installs deps,

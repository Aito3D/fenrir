...
Translates between Spoolman's data model and Fenrir's internal
...
async def update_spool(
    *,
    ...
    # set outside Fenrir are preserved.
    tag_nulled = (
...
async def reset_spool_consumed_counter(
    ...
    The read mapping in ``_map_spoolman_spool`` then derives Fenrir's
...
def _extra_tag(spool: dict) -> str:
    ...
    edited outside Fenrir, and ``.get("tag", "")`` does not default a key
...
async def link_tag_to_spoolman_spool(
    *,
    ...
    async with client.extra_lock(spool_id):
        ...
        # a holder. Fenrir only ever writes a JSON string here; a third party

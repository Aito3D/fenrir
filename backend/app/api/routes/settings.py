...
def check_backup_schema_compatible(sqlite_path: Path, backup_version: str | None = None) -> None:
    ...
    made_by = f"The backup was made by Fenrir {backup_version}, " if backup_version else "The backup "
    raise BackupSchemaIncompatible(
        "This backup cannot be restored by this version of Fenrir. It carries no value for "
...
async def restore_backup(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if backup_version:
            logger.info("Backup was created by Fenrir %s; this install runs %s", backup_version, APP_VERSION)
        if not is_sqlite():
            try:

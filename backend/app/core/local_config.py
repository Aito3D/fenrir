...
- ``read_local_toml`` reads ``/etc/fenrir/local.toml`` (the file the
  appliance setup wizard writes during firstboot with the user's hostname,
  timezone, and locale).
- ``read_ntp_gate`` reads ``/run/fenrir/time-synced`` (the appliance's
...
- On the Fenrir Appliance: both files exist by the time bambuddy.service
...
DEFAULT_PATH = Path("/etc/fenrir/local.toml")
DEFAULT_NTP_GATE_PATH = Path("/run/fenrir/time-synced")
...
def read_ntp_gate(path: Path = DEFAULT_NTP_GATE_PATH) -> TimeSyncState:
    ...
    Wire contract with fenrir-appliance/firstboot/ntp-gate.sh:

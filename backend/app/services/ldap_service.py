"""LDAP authentication service for Fenrir (#794).
...
class LDAPConfig:
    ...
    group_mapping: dict[str, str]  # LDAP group DN -> Fenrir group name
    auto_provision: bool
    ca_cert_path: str  # Path to CA certificate file (empty = skip verification)
    default_group: str  # Fallback Fenrir group assigned when user has no mapped groups (empty = no fallback)
...
def lookup_ldap_user(config: LDAPConfig, username: str) -> LDAPUserInfo | None:
    ...
    flow, where the caller has already been authenticated as a Fenrir admin
...
def resolve_group_mapping(ldap_groups: list[str], group_mapping: dict[str, str]) -> list[str]:
    """Map LDAP group DNs to Fenrir group names.

    Returns list of Fenrir group names that the user should be added to.
    ...
    for ldap_group in ldap_groups:
        fenrir_group = mapping_lower.get(ldap_group.lower())
        if fenrir_group:
            result.append(fenrir_group)
    return result

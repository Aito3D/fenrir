...
class GitLabBackend(GitProviderBackend):
    ...
    async def push_files(
        ...
        try:
            ...
            commit_message = f"Fenrir backup - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
    ...
    async def _create_initial_commit(
        ...
        try:
            ...
            commit_message = f"Initial Fenrir backup - {datetime.now(timezone.utc).strftime('%Y-%m-%d %H:%M:%S UTC')}"
            commit_response = await client.post(
                f"{api_base}/projects/{encoded_path}/repository/commits",
                headers=headers,

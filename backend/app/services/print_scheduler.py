        ...
        async with async_session() as db:
            ...
            for item in items:
                ...
                if item.printer_id:
                    ...
                    if not printer_idle:
                        if self._drying_in_progress.get(item.printer_id) and no_auto_stop and not block_for_drying:
                            ...
                            if (
                                ...
                                # Busy-shaped on purpose: the cycle ends on its own
                                # and the job goes out, so nothing to alert about.
                                await hold_for_printer(
                                    item,
                                    item.printer_id,
                                    "drying, and the print cannot run alongside it",
                    ...
                    # rest of the availability checks — this catches printers
                    # that read idle while a cycle runs.
                    if self._drying_in_progress.get(item.printer_id):
                        if block_for_drying:
                            ...
                            continue
                        # Never-auto-stop mode: the dispatch-time stop below
                        # must never fire, so unless the hardware can dry
                        # through the print, the print waits for the timer.
                        if no_auto_stop and not await self._drying_may_continue_through_print(db, item.printer_id):
                            await hold_for_printer(
                                item,
                                item.printer_id,
                                "drying in never-auto-stop mode, waiting for the timer",
                                f"Busy: {printer_label(item.printer_id)} (drying)",
                            )
                            continue
        ...
        for printer in all_printers.scalars():
            ...
            if not mid_print:
                ...
                if not ambient_drying_enabled and pid not in printers_with_scheduled:
                    if (
                        self._drying_in_progress.get(pid)
                        and not no_auto_stop
                        and pid not in self._scheduled_drying_printer_ids
                    ):

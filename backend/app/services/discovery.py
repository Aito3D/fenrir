...
# Runtimes that put Fenrir in an OCI container whose network mode is a
...
def detect_container_runtime() -> str | None:
    """Name the container runtime Fenrir is running under, or None.
...
# Virtual printer serial suffix to exclude from discovery (Fenrir's own virtual printer)
...
class PrinterDiscoveryService:
    ...
    def _handle_response(self, response: str, ip_address: str):
        ...
        # Skip Fenrir's own virtual printer (any model variant)
        if serial.endswith(VIRTUAL_PRINTER_SERIAL_SUFFIX):
            logger.debug("Ignoring Fenrir virtual printer at %s", ip_address)
            return
...
class SubnetScanner:
    ...
    async def _probe_host(self, ip: str, timeout: float):
        ...
        if serial and serial.endswith(VIRTUAL_PRINTER_SERIAL_SUFFIX):
            logger.debug("Ignoring Fenrir virtual printer at %s", ip)
            return

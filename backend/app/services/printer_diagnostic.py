...
def _host_source_ip(destination_ip: str) -> str | None:
    """The local IPv4 address Fenrir would send from toward ``destination_ip``.
...
def _same_subnet(printer_ip: str, host_ip: str) -> bool | None:
    """Is ``printer_ip`` inside the network configured on Fenrir's ``host_ip``?
...
def _interpreter_is_signed() -> bool | None:
    """Does the interpreter Fenrir runs under carry a code signature?
...
async def run_connection_diagnostic(
    ...
    *,
    ...
    # Not Docker-only: Podman runs Fenrir in exactly the same two shapes and

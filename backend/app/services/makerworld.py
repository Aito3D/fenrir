...
Lets Fenrir resolve a MakerWorld URL, enumerate plate/profile metadata, and
...
Fenrir's existing Bambu Cloud bearer token (same SSO backend — no separate
OAuth flow needed).
...
# as Fenrir with a source URL so Bambu can distinguish our traffic from
...
_CLIENT_HEADERS = {
    "User-Agent": "Fenrir/1.0 (+https://github.com/maziggy/bambuddy)",
...
def _s3_ssl_context() -> ssl.SSLContext:
    ...
    every other network call in Fenrir — verifies against the bundled

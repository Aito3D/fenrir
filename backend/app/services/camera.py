...
async def capture_camera_frame_bytes(
    ...
    # The key is the IP alone (see _inflight_captures), so a follower that asked
    # for a different gpu_accel than the leader gets the leader's frame. That is
    # fine: hwaccel changes how the JPEG is decoded, not what it depicts.
    task = asyncio.create_task(
        _capture_camera_frame_bytes_uncoalesced(ip_address, access_code, model, timeout, gpu_accel=gpu_accel)
    )

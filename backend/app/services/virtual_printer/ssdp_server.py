...
Also provides SSDP proxy functionality for proxy mode, where Fenrir sits
...
class SSDPProxy:
    ...
    header changed to point to Fenrir's IP on LAN B.

    This allows Bambu Studio on LAN B to discover the printer via Fenrir.
    """
    ...
    def _rewrite_ssdp(self, data: bytes) -> bytes:
        ...
        - Location: changed to Fenrir's remote interface IP
    ...
    async def _broadcast_to_remote(self) -> None:
        ...
        try:
            # Rewrite Location to point to Fenrir's remote interface
            rewritten = self._rewrite_ssdp(self._last_printer_ssdp)

<#
.SYNOPSIS
    Fenrir Docker installation script for Windows (Docker Desktop).
...
.PARAMETER InstallPath
    Installation directory. Default: $env:USERPROFILE\fenrir
...
function Write-Banner {
    ...
    Write-Host '   Fenrir - Docker Installation (Windows)' -ForegroundColor Cyan
    Write-Host '==========================================================' -ForegroundColor Cyan
    Write-Host ''
}
...
function Start-Fenrir {
    Info 'Starting Fenrir container...'
    if ($Build) {
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "    $DockerCompose logs fenrir" -ForegroundColor Yellow
        exit 1
    }
    ...
    while ($attempts -lt 15) {
        ...
        if ($ps -match 'Up') { Ok 'Fenrir container is running'; return }
        if ($ps -match 'Exited') {
...
# Defaults
if (-not $InstallPath) { $InstallPath = Read-Default 'Installation directory' (Join-Path $env:USERPROFILE 'fenrir') }
...
Write-Host '==========================================================' -ForegroundColor Green
Write-Host ''
Write-Host "  Access Fenrir:  http://localhost:$Port" -ForegroundColor Cyan
...
Write-Host "    Logs:     cd `"$InstallPath`"; $DockerCompose logs -f fenrir"

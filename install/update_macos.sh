...
repair_python_signature() {
  ...
  for target in "${targets[@]}"; do
    ...
    if codesign --force --sign - "$target" >/dev/null 2>&1; then
      ...
      warn "Could not sign $target; Fenrir may be unable to reach the printer."
      warn "Run by hand: codesign --force --sign - \"$target\""
    fi
  done

  [ "$signed_any" -eq 0 ] || log "Restart any open Fenrir page after this update; the signature changes only take effect on the restart below."
  return 0
}

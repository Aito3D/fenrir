#!/usr/bin/env bash
#
# Fenrir Native Installation Script
...
#   Unattended:   ./install.sh --path /opt/fenrir --port 8000 --yes
#
# Options:
...
show_help() {
    echo "Fenrir Native Installation Script"
    echo ""
    echo "Usage: $0 [OPTIONS]"
    echo ""
    echo "Options:"
    echo "  --path PATH        Installation directory (default: /opt/fenrir)"
    ...
    echo "    ./install.sh --path /srv/fenrir --port 3000 --tz America/New_York --yes"
    echo ""
...
download_fenrir() {
    ...
    if ! git ls-remote --exit-code --heads https://github.com/maziggy/bambuddy.git "$BRANCH" &>/dev/null; then
        log_error "Branch '$BRANCH' not found in the Fenrir repository."
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        # user (not root, not fenrir) unable to write .git into it.
        sudo mkdir -p "$INSTALL_PATH"
        sudo git clone --branch "$BRANCH" https://github.com/maziggy/bambuddy.git "$INSTALL_PATH"
        sudo chown -R "$SERVICE_USER:$SERVICE_USER" "$INSTALL_PATH" 2>/dev/null || true
    fi

    log_success "Fenrir downloaded to $INSTALL_PATH (branch: $BRANCH)"
}
...
sign_python_for_tcc() {
    ...
    for target in "${targets[@]}"; do
        ...
        if codesign --force --sign - "$target" &>/dev/null; then
            ...
            log_info "Fenrir may be unable to reach the printer. Run this by hand:"
...
EOF
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        mv /tmp/fenrir.env "$env_file"
        chmod 600 "$env_file"
    else
        sudo mv /tmp/fenrir.env "$env_file"
...
EOF
    ...
    if prompt_yes_no "Enable Fenrir to start on boot?" "y"; then
        sudo systemctl enable fenrir
        log_success "Service enabled"
    fi

    if prompt_yes_no "Start Fenrir now?" "y"; then
        sudo systemctl start fenrir
        sleep 2
        if sudo systemctl is-active --quiet fenrir; then
            log_success "Fenrir is running"
        else
            log_warn "Service may have failed to start. Check: sudo journalctl -u fenrir -f"
        fi
    fi
}
...
create_launchd_service() {
    ...
    local plist_path="$HOME/Library/LaunchAgents/com.fenrir.app.plist"
...
<dict>
    ...
    <string>$LOG_DIR/fenrir.error.log</string>
</dict>
</plist>
EOF
    ...
    if prompt_yes_no "Load Fenrir service now?" "y"; then
        launchctl load "$plist_path"
        sleep 2
        if launchctl list | grep -q "com.fenrir.app"; then
            log_success "Fenrir is running"
        else
            log_warn "Service may have failed to start. Check: cat $LOG_DIR/fenrir.error.log"
        fi
    fi
}
...
main() {
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        if [[ "$DEFAULT_INSTALL_PATH" == "/opt/fenrir" ]]; then
            DEFAULT_INSTALL_PATH="$HOME/fenrir"
        fi
    fi
    ...
    if [[ "$BIND_ADDRESS" == "0.0.0.0" ]]; then
        ...
        echo -e "  ${BOLD}Access Fenrir:${NC}  ${CYAN}http://localhost:$PORT${NC}"
    ...
    if [[ "$OS_TYPE" == "macos" ]]; then
        ...
        echo -e "    Start:   launchctl load ~/Library/LaunchAgents/com.fenrir.app.plist"
        echo -e "    Stop:    launchctl unload ~/Library/LaunchAgents/com.fenrir.app.plist"
        echo -e "    Logs:    tail -f $LOG_DIR/bambuddy.log"
    else
        echo -e "  ${BOLD}Manage service:${NC}"
        echo -e "    Status:  sudo systemctl status fenrir"
        echo -e "    Start:   sudo systemctl start fenrir"
        echo -e "    Stop:    sudo systemctl stop fenrir"
        echo -e "    Logs:    sudo journalctl -u fenrir -f"
    fi
    echo ""
    echo -e "  ${BOLD}Update Fenrir:${NC}"
    ...
    if [[ "$OS_TYPE" != "macos" ]]; then
        echo -e "    sudo systemctl restart fenrir"
    fi
    echo ""

#!/usr/bin/env bash
#
# Fenrir Docker Installation Script
...
#   Unattended:   ./docker-install.sh --path /opt/fenrir --port 8000 --yes
#
# Options:
...
show_help() {
    echo "Fenrir Docker Installation Script"
    echo ""
    ...
    echo "    ./docker-install.sh --path /srv/fenrir --port 3000 --tz America/New_York --yes"
    echo ""
...
create_install_dir() {
    ...
    if ! mkdir -p "$INSTALL_PATH" 2>/dev/null; then
        # The default `/opt/fenrir` (and any other root-owned parent) needs
...
start_container() {
    ...
    while [[ $attempt -lt $max_attempts ]]; do
        ...
        if $DOCKER_CMD ps | grep -q "Up"; then
            log_success "Fenrir container is running"
            return 0
        fi
        ...
        if $DOCKER_CMD ps -a | grep -q "Exited"; then
            ...
            log_info "Check logs with: $DOCKER_CMD logs fenrir"
            return 1
        fi
...
main() {
    ...
    echo -e "    Logs:    cd $INSTALL_PATH && $DOCKER_CMD logs -f fenrir"

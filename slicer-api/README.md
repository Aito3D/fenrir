...
OrcaSlicer and/or Bambu Studio CLI. Fenrir's **Slice** action calls
these to slice models server-side, no desktop slicer required.

This folder is **optional**. Fenrir works without it — Slice falls back
...
the sidecar on a separate x86_64 box and point Fenrir at it via the
**Sidecar URL** field — the sidecar doesn't need to live next to Fenrir.
...
|---|---|---|
| `orca-slicer-api` | **3003** | Fenrir's virtual-printer feature reserves 3000 and 3002 |
...
In the Fenrir UI: **Settings → Slicer**:
...
`BAMBU_STUDIO_API_URL` environment defaults from Fenrir's config.

## Where the images live

Pre-built images are published to two registries on every Fenrir
stable release:
...
Each release also publishes a versioned tag (`:fenrir-X.Y.Z`) so you
can pin to the sidecar that shipped alongside a specific Fenrir
release — set `SIDECAR_TAG=fenrir-0.2.5` in `.env`.

Both images are built from the
[`maziggy/orca-slicer-api`](https://github.com/maziggy/orca-slicer-api)
fork (`fenrir/profile-resolver` branch). The fork patches AFKFelix's
...
To roll back to the sidecar that shipped with a previous Fenrir
release, set `SIDECAR_TAG=fenrir-X.Y.Z` in `.env` and re-run the two
commands above.
...
- **`address already in use` on port 3000 or 3002** — Fenrir's

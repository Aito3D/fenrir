...
scan_codeql_python() {
    ...
    gh codeql database create --overwrite --language=python --threads=0 /tmp/fenrir-codeql-python &>/dev/null
    echo "Analyzing..."
    gh codeql database analyze /tmp/fenrir-codeql-python \
...
scan_codeql_js() {
    ...
    echo "Creating database..."
    gh codeql database create --overwrite --language=javascript --source-root=frontend --threads=0 /tmp/fenrir-codeql-javascript &>/dev/null
    echo "Analyzing..."
    gh codeql database analyze /tmp/fenrir-codeql-javascript \
...
scan_codeql_actions() {
    ...
    echo "Creating database..."
    gh codeql database create --overwrite --language=actions --threads=0 /tmp/fenrir-codeql-actions &>/dev/null
    echo "Analyzing..."
    gh codeql database analyze /tmp/fenrir-codeql-actions \
...
scan_trivy_image() {
    ...
    docker build -t fenrir:security-scan . 2>&1
    echo ""
    trivy image --severity CRITICAL,HIGH,MEDIUM fenrir:security-scan 2>&1
}
...
echo -e "${BOLD}Fenrir Security Scanner${NC}"
